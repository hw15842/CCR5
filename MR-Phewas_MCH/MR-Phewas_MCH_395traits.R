...
readfile <- paste(Trait, "filtered_sumstats.txt", sep="_")

MCH_all_data <- read.table(readfile, header=T)
...
savefile2 <- paste (Trait, "clumping_results_all.txt", sep="_")

write.table(results_all, file=savefile2, quote=F, sep="\t")
...
savefile3 <- paste (Trait, "results_MRbase", Trait, ".txt", sep="_")

write.table(results_MRbase_MCH_table, file = savefile3, quote=F, sep="\t")
